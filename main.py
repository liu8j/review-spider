...
options.binary_location = r"C:\Program Files\Google\Chrome\Application\chrome.exe"  # 指定chrome程序路径
...
options.add_argument("--user-data-dir=D:\\Data")  # 加载配置启动浏览器，不然就以一个全新的浏览窗口启动
options.add_experimental_option("detach", True)
s = Service("chromedriver.exe")
driver = webdriver.Chrome()
driver.implicitly_wait(10)  # 隐式等待。网页加载数据需要时间，智能化等待。
driver.get('http://www.etmoc.com/Firms/Brands?page=1')
...
def get_info():
    ...
    price_buy = [p.find_element(By.CLASS_NAME, "ciga_list_list_item_bar_price_t").text.replace("批发价：", "") for p in
                 price]
    price_sale = [p.find_element(By.CLASS_NAME, "ciga_list_list_item_bar_price_b").text.replace("建议零售价：", "") for p
                  in price]
...
if __name__ == "__main__":
    while True:
        ...
        WebDriverWait(driver, 10).until(
            EC.visibility_of_all_elements_located((By.CLASS_NAME, "ciga_list_list_item_img")))
        get_info()
